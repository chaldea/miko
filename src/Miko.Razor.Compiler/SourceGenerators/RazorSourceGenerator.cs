...
#pragma warning restore RS1041 // This compiler extension should not be implemented in an assembly with target framework '.NET 8.0'. References to other target frameworks will cause the compiler to behave unpredictably.
    public partial class RazorSourceGenerator : IIncrementalGenerator
    {
        ...
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            var routeRegistrationSource = csharpDocuments
                .Collect()
                .Combine(razorSourceGeneratorOptions)
                .Combine(isGeneratorSuppressed)
                .Select(static (pair, _) =>
                {
                    var ((documents, options), isGeneratorSuppressed) = pair;
                    if (isGeneratorSuppressed)
                    {
                        return (string?)null;
                    }

                    var routes = new List<(string fullTypeName, string routeTemplate)>();

                    foreach (var (_, codeDocument, _) in documents)
                    {
                        ...
                        var cls = documentNode.FindPrimaryClass();
                        if (ns?.Name == null || cls?.Name == null)
                        {
                            continue;
                        }

                        var fullTypeName = $"{ns.Name}.{cls.Name}";

                        foreach (var child in ns.Children)
                        {
                            if (child is RouteAttributeExtensionNode routeNode)
                            {
                                var template = routeNode.Template.Trim('"');
                                routes.Add((fullTypeName, template));
                            }
                        }
                    }
                    ...
                    sb.AppendLine("{");
                    sb.AppendLine("    internal static class GeneratedRouteRegistration");
                    sb.AppendLine("    {");
                    sb.AppendLine("        public static void RegisterRoutes(global::Miko.Routing.Router router)");
                    sb.AppendLine("        {");

                    foreach (var (fullTypeName, routeTemplate) in routes)
                    {
                        sb.AppendLine($"            router.MapRoute<global::{fullTypeName}>(\"{routeTemplate}\");");
                    }

                    sb.AppendLine("        }");
                    sb.AppendLine();
                    sb.AppendLine("        public static global::Miko.Hosting.MikoAppBuilder UseGeneratedRoutes(this global::Miko.Hosting.MikoAppBuilder builder)");
                    sb.AppendLine("        {");
                    sb.AppendLine("            builder.UseRouter(RegisterRoutes);");
            ...
            context.RegisterImplementationSourceOutput(routeRegistrationSource, static (context, source) =>
            {
                if (source != null)
                {
                    context.AddSource("GeneratedRouteRegistration.g.cs", source);
                }
            });

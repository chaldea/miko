...
let activState: any = undefined;
let deactivState: any = undefined;
export function onRouteStateChange(instance, isActive) {
  if (isActive) {
    activState = instance;
    let lastLocation = location.href;
    let isBackwards = false;
    let invokeTransition = () => {
      activState?.invokeMethodAsync('Navigate', isBackwards);
      deactivState?.invokeMethodAsync('Navigate', isBackwards);
    }
